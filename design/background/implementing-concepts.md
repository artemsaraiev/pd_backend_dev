...
**Important distinction: `_id` vs external identifiers**
- `_id`: MongoDB's unique document identifier, **always** generated by the backend using `freshID()` when creating new documents
- External identifiers: Some concepts may use external identifiers (like DOI, arXiv IDs, ORCID, etc.) that come from outside the system. These should be stored in a separate field (e.g., `paperId`), not used as the `_id`. Both `_id` and the external identifier should be stored separately.

For example, in PaperIndex, papers have both an internal `_id` (generated with `freshID()`) and a `paperId` field (external identifier like DOI/arXiv):
...
When implementing `ensure` or similar actions that accept external identifiers, you should:
1. Check if a document with the given external identifier already exists (by querying the external identifier field)
2. If it exists, return its internal `_id`
3. If it doesn't exist, create a new document with a fresh `_id` (using `freshID()`) and store the external identifier in the separate field
