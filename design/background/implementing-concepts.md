...
- `_id`: MongoDB's unique document identifier, typically generated by the backend using `freshID()` when creating new documents
- External identifiers: Some concepts may use external identifiers (like DOI, arXiv IDs, ORCID, etc.) that come from outside the system. In such cases, you may use the external identifier as the `_id` for efficiency, but you should also store it in an explicit field to match the spec and clarify the distinction.

For example, in PaperIndex, papers have a `paperId` field (external identifier like DOI/arXiv) which is also used as the `_id`:
